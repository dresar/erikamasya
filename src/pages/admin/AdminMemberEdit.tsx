...
const AdminMemberEdit = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/members/${memberId}`)
      ...
      .then((m) => {
        ...
        setNim(String(m.nim ?? ""));
        setAngkatan(String(m.angkatan ?? ""));
        setJurusan(String(m.jurusan ?? ""));
        setStatus(String(m.status ?? "Aktif"));
        setEmail(String(m.email ?? ""));
        setPhone(String(m.phone ?? ""));
        setAddress(String(m.address ?? ""));
        setPlaceOfBirth(String(m.placeOfBirth ?? ""));
        setDateOfBirth(String(m.dateOfBirth?.slice?.(0, 10) ?? ""));
        setGender(String(m.gender ?? ""));
        setJoinedAt(String(m.joinedAt?.slice?.(0, 10) ?? ""));
        setNotes(String(m.notes ?? ""));
        setPhotoUrl(m.photoUrl ?? null);
        setIsActive(Boolean(m.isActive ?? true));
      })
      .catch(() => setError("Gagal mengambil data anggota"))
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate(`/admin/anggota/${memberId}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-foreground mb-1">Edit Anggota</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Perbarui identitas, kontak, dan informasi tambahan.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-end">
              <Button disabled={isLoading || isSaving || isUploading} onClick={onSave}>
                Simpan
              </Button>
            </div>
          </CardContent>
